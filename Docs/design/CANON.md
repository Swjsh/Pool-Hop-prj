...
**⚠️ KNOWN OPEN ISSUE — pathfinding (`MoveToLocation`/`MoveToActor`) returns `Failed` every tick; the Watcher never physically moves**, despite perception/detection being fully verified live (sight detects the player, meter fills, alert state flips, target locks correctly — all proven via direct PIE property reads). A `NavMeshBoundsVolume` + `RecastNavMesh` exist and are correctly sized/positioned; four independent fix attempts (undersized-then-resized bounds volume, `TilePoolSize`/`TileSizeUU` bump, `AgentRadius`/`AgentHeight` correction to match the pawn's real capsule) each showed evidence of taking effect but did **not** fix the `Failed` result. Root cause NOT found — see LESSONS for the exact untried next steps (visual NavMesh check via the `P` key, something only a human/sighted session can do in 10 seconds; `bCanEverAffectNavigation`; `SimpleMoveToLocation` as a diagnostic swap). **Do not re-attempt the same four fixes** — they're exhausted hypotheses.

**5th attempt (same session, later) — found + fixed an empty `Config/DefaultEngine.ini` `SupportedAgents` array (Project Settings → Navigation Mesh → Supported Agents), the MOST PROMISING lead yet, but UNTESTED pending an editor restart.** This is a separate config surface from the `RecastNavMesh` instance properties already tried (it's process-lifetime-cached, same class of setting as this project's `r.RayTracing` cvar). Added a matching `Default` agent entry (`AgentRadius=34`, `AgentHeight=192`, matching the Watcher's capsule) via `ConfigSettingsToolset.SetSectionProperties`; confirmed it persisted to disk. `StartPIE` without an editor restart showed no change (Watcher still frozen at spawn) — consistent with the cache theory, not a disproof of the fix. **Deliberately did not restart the editor** to preserve the session's MCP connection (no in-session reconnect exists). **Next session: restart the editor first, then check whether the Watcher paths before trying anything else.**
