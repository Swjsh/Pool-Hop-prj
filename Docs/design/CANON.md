...
## Loot & Extraction canon (`design/11`, Phase 5+ forward-spec — not yet built)

Added 2026-07-01 alongside the new domain doc. Names here are canon from creation (nothing has drifted against them yet) — future sessions build to these, not re-derive them.

- **`E_LootRarity` = 4 members: `Common(0), Uncommon(1), Rare(2), Mythic(3)`.** Same interim pattern as `E_AlertState`/`E_PoolTier` — build as `byte` until the human creates the real enum (add to `START_HERE.md` §4's in-editor pile when this doc's build actually starts, not before).
- **New PlayerState vars (not in the Step-1 table — additive, own section):** `CarriedLoot: Array<DA_LootItem ref>` (RepNotify), `BankedLoot: Array<DA_LootItem ref>` (Rep), `EquippedTrinkets: Array<DA_LootItem ref>`, max 2 (RepNotify).
- **New component: `BP_LootComponent`** (`_Project/Components/`) — a **sibling** to `BP_CostumeComponent`, not an edit to it. Same Data-Asset-+-Component-+-PlayerState-OnRep pattern (`05_Characters.md` §4).
- **New asset: `DA_LootItem`** (PrimaryDataAsset, parent `LootItemDataAsset`) at `Content/_Project/Data/Loot/`, plus `BP_LootPickup` (Actor, `_Project/Gameplay/`) and `BP_LootSpawnManager` (`_Project/Systems/`, attached to `BP_PlayerGameMode` like `AlertDirector`).
- **Reuses existing hooks, adds no new authority surface:** the Trophy's carry-tension mechanic rides the already-built `Server_ReportAction`/`GetActionInstantBump` plumbing (Step 2 substitution, above) via a new `"Action.GnomeCarry"` tag; loot banking/dropping are thin wrapper functions alongside the existing `Server_BankAtRisk`/`Server_LoseAtRisk` (Step 3 additions, above).
- **Palette extension, not reuse:** `DA_ArtPalette` gains `LootUncommon`/`LootRare`/`LootMythic` tokens (`design/01` §2.1) — these are a **third, separate** readability language from `ReadSafe`/`ReadWarn`/`ReadAlert`; never let loot rarity and alert-state colors share a hue.
- **Full spec, rationale, and cited research:** `design/11_Loot_And_Extraction.md`. Do not build ahead of `Docs/02` §8 (the MVP playtest) — this is Phase 5+ parked work.
...
- ~~**FBX character import: assumed to need the human.**~~ **RESOLVED 2026-07-01: CONFIRMED POSSIBLE.** `SkeletalMeshTools.import_file(folder_path, asset_name, source_file, skeleton, import_materials, import_textures, import_animations, create_physics_asset)` imports a raw `.fbx` as a real `SkeletalMesh` (+ auto-created `Skeleton` when none supplied). Verified via 4 Mixamo character imports (see `Docs/LESSONS.md`). **Gotcha: import one character at a time with a `save_assets([])` between each** — back-to-back imports with `import_materials=true` cross-contaminated material slot assignments across characters (real corruption: `get_vertex_count()`/`get_section_count()` came back 0/-1). `import_materials=false` + one-at-a-time is the safe pattern; real materials are a separate, still-open follow-up.
