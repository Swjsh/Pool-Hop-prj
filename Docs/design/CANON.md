...
**`BP_SensorLight`** (`_Project/Gameplay/`, one placed near the sandbox's existing pool/Watcher area — Pool B doesn't exist yet per Step 3, so it's not literally "near Pool B" yet, revisit once Step 5's fence dressing adds Pool B) per `04 §10`: `TriggerBox` (BoxComponent, collision profile `Trigger`) + `SensorSpotLight` (SpotLightComponent, off by default via `bVisible=false`). On player overlap (HasAuthority + tag check): light on, nearest Watcher gets `bHeardNoise=true`+`LastKnownLocation`, `GameMode.Server_NotifySensorTrip(Location)` (a thin wrapper reaching `AlertDirector.NotifySensorTrip`), `SetTimerByFunctionName` → `TurnOffLight` after `LightOnDuration=4.0s`.

**⚠️ Not yet independently verified: the exact heat MAGNITUDE from the Alert-tier (+25) bump.** Graph wiring is exhaustively confirmed correct via `get_node_infos` (see LESSONS); the Suspicious-tier (+8) bump reliably lands in testing, but the Alert-tier bump's observed magnitude was consistently lower than expected across several live-PIE test cycles. Root cause not found before the MCP connection dropped mid-diagnosis — a temporary `PrintString` diagnostic is left in `PollWatcherAndPush` for the next session to read via the log. **Do not trust `AccumulatedHeat`'s exact Alert-tier value until this is resolved.**
