...
7. 🔨 **Step 5 AlertDirector/sensor light/caught-detain — built, mostly verified (new session).** `BP_AlertDirectorComponent` (heat accrual + decay + thresholds → `GameState.NeighborhoodHeat`/`AlertLevel`), `BP_SensorLight` (trigger + spotlight + heat bump), `GameMode.HandleDetain`/`FinishDetain` (caught → lose at-risk → respawn at stash). See CANON's "Step 5 substitution" for the full build (including two new MCP-buildability gaps found: cross-object accessor indexes go permanently stale per-already-touched-graph — not just per-fresh-target — and the built-in `Delay` node can't be created via MCP at all). **⚠️ One open item:** the Alert-tier (+25) heat bump's exact magnitude is unconfirmed — a diagnostic `PrintString` was left in `BP_AlertDirectorComponent.PollWatcherAndPush` when the MCP connection dropped mid-diagnosis. **Next session: read the PIE log for `DIAG` lines during a fresh Watcher-detection cycle (`EditorToolset.LogsToolset`) to see the exact tick-by-tick state sequence, root-cause it, then remove the diagnostic.** See LESSONS' "UNRESOLVED" entry for the full trail (what's already ruled out).
8. Then finish roadmap Step 6 (costume) → 7 (HUD, human authors UMG) → 8 (the playtest that decides if it's fun).
