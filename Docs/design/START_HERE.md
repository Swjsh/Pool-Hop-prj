...
- ⚠️ **Two small movement follow-ups remain** (next session): **Crouch (`C`→IA_Crouch)** and **Sprint (`LeftShift`→IA_Sprint)** are not yet mapped in the restored `IMC_Default` (the template IMC we restored lacks them). Re-add via MCP `set_properties`, then **verify on disk** (cold launch + `GetAll InputMappingContext Mappings`). And `BP_PlayerController`'s BeginPlay bytecode is stale on disk (never runs from a cold load — harmless now that input is applied via config `DefaultMappingContexts`, but recompile+resave it to be clean).
...
3. **Finish System 1 input:** re-add Crouch + Sprint mappings to `IMC_Default` via MCP; recompile+resave `BP_PlayerController`; **verify on a cold standalone launch** (`unreal-input-probe`).
4. **Step 3 scoring logic** (MCP-buildable, do it with the human watching the score number in PIE): wire `BP_PoolVolume` overlap → `PoolScoreComp` → `GameMode.Server_AddScore` → PlayerState/GameState at-risk; `BP_StashZone` → bank. Full spec `06 §3`, sequence in roadmap Step 3.
5. **Step 2 finish:** wire the loudness action sources + `DT_LoudnessActions`.
6. **Step 4 AI Watcher** (the priority deliverable): you build the perception + Blueprint logic via MCP; the **human authors `BT_Watcher`/`BB_Watcher`** (BT assets aren't MCP-authorable — give them click-by-click steps from `04 §12`).
7. Then roadmap Steps 5 (couple) → 6 (costume) → 7 (HUD, human authors UMG) → 8 (the playtest that decides if it's fun).
...
3. **Author `BT_Watcher` + `BB_Watcher`** (Step 4) — you build everything around it; the human clicks the tree together from `04 §12`.
