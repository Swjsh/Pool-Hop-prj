...
> **What this doc is.** The single north-star document for Pool Hop's design package. It states the fantasy, the pillars, and the core loop, then shows how the seven domain design docs (`01`–`07` in this `Docs/design/` folder) interlock into one coherent game. Read this first; then dive into whichever domain doc owns the piece you're building. For the *ordered build plan* that turns all of this into editor work, see the companion `08_Implementation_Roadmap.md`.
>
