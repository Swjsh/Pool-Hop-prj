...
Spiked + confirmed (`Docs/LESSONS.md`, `Docs/design/CANON.md` §"Known MCP buildability gaps"). The *dedicated toolsets* **cannot** create: **Blueprint Enums/Structs**, **Run-on-Server/Reliable RPC flags**, **Behavior Trees/Blackboards**, and (unconfirmed) EQS. Interim substitutes are canon: enums → `byte`, `Server_*` RPCs → regular `HasAuthority`-guarded functions, BT → Blueprint state machine (`TickBrain`).

**⚡ BUT the ceiling is much lower than "no toolset" implies — the editor Python console has the FULL `unreal` API (§7 below).** IK Rigs / IK Retargeters / batch anim retargeting (Mixamo→Mannequin) have **no dedicated toolset** yet were built with zero human clicks via the Python console (2026-07-02, `Docs/LESSONS.md` top entry — `IKRigController`/`IKRetargeterController`/`IKRetargetBatchOperation`). Before declaring anything "needs the human," check whether the Python `unreal` API can do it. computer-use can't reach the editor window here, but the Python console largely replaces that need.

## 7. The editor Python console — full `unreal` API, no toolset needed

Type `py "C:/abs/path/script.py"` into the **status-bar Cmd box** (SlateInspector widget ref `tb2`: `SlateInspectorToolset...Type {ref:"tb2", text:"py \"...\"", submit:true}` — bring the main window front first if `Type`/`Click` returns false: `Windows {action:"select", index:0}`). The script runs in-editor with the full `unreal` module (asset factories, `IKRigController`, `IKRetargeterController`, `GameplayStatics`, `EditorAssetLibrary`, subsystem access, etc.) — unlike `execute_tool_script`, which is stdlib-only and can't `import unreal`. Read results back from `Saved/Logs/PoolHop.log` (`unreal.log("[TAG] ...")`). Gotchas: `save_loaded_asset` no-ops unless `only_if_is_dirty=False`; off-screen `get_bone_transform` returns ref pose (use Simulate `CaptureViewport` to eyeball a pose); a retargeted UE-template ABP carries the source Control Rig and stamps a T-pose (set its AnimGraph ControlRig node `Alpha=0`).
