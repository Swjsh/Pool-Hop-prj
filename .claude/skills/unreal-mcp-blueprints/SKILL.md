...
## `get_node_type_pins()`'s auto-created preview node is NOT safe to wire into — use `create_node` instead

**Correction to an earlier claim in this file:** `get_node_type_pins(graph, type_id)` DOES create a real, persistent node in the graph as a side effect (this is how you get a real node ref to read pin indices from) — but that node does not always share the same graph "outer" as legitimately-placed nodes. Wiring `connect_pins` between EventGraph nodes and a `get_node_type_pins`-created node (e.g. a `Utilities|FlowControl|Branch`) compiled to a hard failure: a wall of `ICE SetVariableOnPersistentFrame - No property found` errors covering EVERY event's data pins in the whole Ubergraph (not just the touched ones), plus a genuine engine `ensure` — `UEdGraphPin::MakeLinkTo Pin 'execute' on node 'Branch' has a different outer than pin 'then' on node 'Assign'`. **Fix: call `create_node(graph, type_id, pos)` explicitly for any node you intend to wire into** — same type_id, properly parented, compiles clean. Use `get_node_type_pins` ONLY for schema discovery (reading pin names/indices/types), never as the actual node you connect to. (LESSONS 2026-07-02, `BP_PlayerCharacter` crouch-toggle rewrite — hit this on the very first attempt, recovered via the reload-packages technique below, redid cleanly with explicit `create_node`.)

## Recovering from a bad in-memory edit with ZERO disk risk

If a Blueprint edit produces a **hard compile failure** (not just a warning) and the asset has **not been saved this session**, the safest recovery is discarding the in-memory changes entirely rather than trying to patch forward through a possibly-corrupted graph:
```python
pkg = unreal.load_asset(path).get_package()
unreal.EditorLoadingAndSavingUtils.reload_packages([pkg], unreal.ReloadPackagesInteractionMode.ASSUME_POSITIVE)
```
`ASSUME_POSITIVE` (not `INTERACTIVE`, which blocks on a modal dialog the MCP can't dismiss) silently answers "yes, discard and reload from disk" with zero prompts. Verify the recovery actually worked — don't just trust a lack of error: re-run `compile_blueprint` (should succeed clean) and `get_node_infos` on the nodes you'd touched (should show the original, pre-edit wiring). This is now the standing move for any hard compile error on a not-yet-saved asset, especially a shared/critical file — don't risk saving a half-fixed graph.

## `write_graph_dsl` leaves the PREVIOUS version's nodes behind — re-editing the same function accumulates dead nodes silently

Rewriting a function via `write_graph_dsl` does not delete what it's replacing — it builds a new node chain and repoints `FunctionEntry`'s exec pin to it, leaving the old chain physically in the graph, fully disconnected (no incoming exec edge), and invisible to `compile_blueprint(warnings_as_errors=true)` (UE does not warn on this). `find_nodes` returns BOTH the live and the dead nodes with no indication of which is which. Across 2-3 prior-session rewrites of the same function this compounded to ~2x the real node count (LESSONS 2026-07-02, `BP_LoudnessComponent.Server_ReportAction`: 35 nodes returned, only 18 reachable from `FunctionEntry`). `get_node_type_pins`'s schema-discovery probe nodes DO also persist in the graph (correction to an earlier version of this note) — see the dedicated gotcha above for why that's actually the more dangerous of the two (orphaned DSL nodes are inert; a wired-into probe node can corrupt the whole graph's compile).
...
- **A DSL `(if (Utilities|IsValid x) …)` guard does NOT reliably lower to a working `IsValid → Branch → …` shape.** `IsValid` is a K2 macro; the DSL left both macro exec outputs (`Is Valid`/`Is Not Valid`) unconnected — a dead-end — while orphaning the guarded nodes (they compiled but were unreachable from the entry). `read_graph_dsl` hides this (renders a bare `(Utilities|IsValid …)`); only `get_node_infos` on the macro shows the unwired outputs. Same family as the `IsValid`-inside-`and` breakage (catch/detain, 2026-07-02). **If you need a null guard: either wire a `K2Node_MacroInstance` IsValid surgically, or — the robust trick — reorder so the load-bearing work runs FIRST and the fragile/guarded work runs LAST, so a mis-lowered guard dead-ends only the expendable tail** (used to make `UpdateVisionConeParams` error-proof: icon path first, decal-param sets last/removed). Each `write_graph_dsl` rewrite also leaves the previous version's nodes as orphans — see the orphan-accumulation section above; they're harmless once fully disconnected (zero compile warnings) but clean them opportunistically.
