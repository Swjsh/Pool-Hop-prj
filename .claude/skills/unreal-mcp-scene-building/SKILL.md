...
- **Measuring at runtime — screenshots and the log frame counter LIE; use these instead (learned the hard way, LESSONS 2026-07-01 PM):**
  ...
  - **`CaptureEditorImage` (window grab) returns identical/frozen frames when the editor is NOT OS-foreground** (the editor throttles its window paint to ~3 fps). Automation can't reliably force OS foreground (Windows blocks `SetForegroundWindow` from background procs). Don't conclude "frozen/slow" from a `CaptureEditorImage`. **BUT `CaptureViewport` with an explicit `captureTransform` renders a fresh scene frame regardless of OS focus** (confirmed 2026-07-02, incl. during `bSimulate:true` — `was_recently_rendered` returned True and successive captures showed live scene state). So visual verification IS available off-foreground — just use `CaptureViewport`, not `CaptureEditorImage`.
  - **Verify a skeletal mesh actually ANIMATES (not a static T-pose) without a human — the reliable recipe (2026-07-02, LESSONS):** off-screen `SkeletalMeshComponent.get_bone_transform(name)` reads LIE (they return the reference pose — the CPU bone array isn't refreshed during synchronous reads). Instead: `StartPIE(bSimulate:true)` → freeze the pawn so it idles in place (`character_movement.max_walk_speed=0` + `stop_movement_immediately()` via the editor Python console) → read its live location → aim `CaptureViewport` at it (~500 units back, chest height, 3/4 angle) → **read the silhouette: arms straight out to the sides = T-pose (broken); arms hanging down = idle animation (working).** Two captures a few seconds apart confirm continuous motion. This is how the Mixamo BigVegas T-pose bug was caught and the fix confirmed.
  - **The editor GPU also idles off-foreground** — `nvidia-smi` `clocks.current.graphics` ~2800 MHz = rendering; ~1852 MHz + ~3% util = throttled/idle. (A `CaptureViewport` forces a render even so.)
...
- **`ActorTools.set_actor_transform` (teleport) proves overlap/scoring LOGIC works, but is blind to collision-blocking bugs a real walking/swimming player would hit.** A hard teleport can place a pawn inside solid geometry without triggering the same collision resolution a player moving under their own steam would — so "teleported the pawn into the pool, watched score climb" proves the Blueprint logic is correct, but says nothing about whether a real player could ever physically reach that space. Caught this exact gap in this project (LESSONS 2026-07-02): all 4 Maple Court pool water surfaces had blocking collision sitting squarely in the entry path, undetected through many teleport-based "verified live in PIE" passes. If a system's only verification path is teleport-based, phrase claims as "the logic is correct," not "a player can reach this" — and separately check collision profiles (`bodyInstance.collisionProfileName`) on anything decorative a player is meant to walk/swim through.
