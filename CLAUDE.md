...
**RESOLVED (2026-07-01 evening — MCP reconnected, diagnosed live).** With editor control back: (1) Manny now **renders correctly** (repaired skeleton/materials). (2) **Movement PROVEN** — drove an auto-walk probe and read the pawn's `CollisionCylinder.RelativeLocation`: it walked from spawn `x=100` to `x=645` (~545 units). Input wiring + `IMC_Default` application were already verified. (3) **The "freeze" = Unreal's not-foreground throttle, not a perf bug** — the GPU idles at 3% util / 1852 MHz when the editor window isn't the OS-foreground app, and boosts to full 2800 MHz when it is (grey box ≈ 10% util). My screenshots only *looked* frozen because automation can't hold OS focus (Windows blocks `SetForegroundWindow` from background procs); the pawn moved underneath. **CONFIRMED FIX:** the freeze was `bThrottleCPUWhenNotForeground` = **True at runtime** (my `.ini` edits never applied). Set it False directly on the CDO — `ObjectTools.set_properties("/Script/UnrealEd.Default__EditorPerformanceSettings", '{"bThrottleCPUWhenNotForeground": false}')` — and **verified the pawn moves during a 5 s *unfocused* wait** (x −1025→−1464). The game now ticks/renders regardless of focus; the freeze is gone. All probes removed, WalkSpeed restored to 600, committed. Persists via the editor's config-save-on-close + the project `Config/DefaultEditorPerProjectUserSettings.ini`. **Remaining polish (not blockers):** assign `ABP_Unarmed` to `CharacterMesh0.AnimClass` so Manny animates instead of sliding in A-pose. **Measurement rule (see LESSONS):** the log `[N]` frame counter is unreliable — measure movement via pawn `RelativeLocation`, perf via `nvidia-smi`.
