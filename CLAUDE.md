...
**RESOLVED (2026-07-01 evening — MCP reconnected, diagnosed live).** With editor control back: (1) Manny now **renders correctly** (repaired skeleton/materials). (2) **Movement PROVEN** — drove an auto-walk probe and read the pawn's `CollisionCylinder.RelativeLocation`: it walked from spawn `x=100` to `x=645` (~545 units). Input wiring + `IMC_Default` application were already verified. (3) **The "freeze" = Unreal's not-foreground throttle, not a perf bug** — the GPU idles at 3% util / 1852 MHz when the editor window isn't the OS-foreground app, and boosts to full 2800 MHz when it is (grey box ≈ 10% util). My screenshots only *looked* frozen because automation can't hold OS focus (Windows blocks `SetForegroundWindow` from background procs); the pawn moved underneath. Set `t.IdleWhenNotForeground=0` + `bThrottleCPUWhenNotForeground=False`. **For the user:** play with the game window as the active/focused window and press W → Manny walks. All probes removed; committed. **Still open:** if *focused* in-viewport play still stutters, verify the editor throttle is actually honored for PIE (may need a different mechanism); and assign `ABP_Unarmed` so Manny animates instead of sliding. **Measurement rule (see LESSONS):** the log `[N]` frame counter is unreliable — measure movement via pawn `RelativeLocation`, perf via `nvidia-smi`.
